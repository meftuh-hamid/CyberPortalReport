...
using AspNetCoreReporting.PredefinedReports;
using AspNetCoreReporting.Data;
using DevExpress.XtraReports.Parameters;
using System.Web;

namespace AspNetCoreReporting.Services
{
        ...
        DevExpress.XtraReports.Web.Extensions.ReportStorageWebExtension
    {
        ...
        public CustomReportStorageWebExtension(ReportDbContext dbContext)
        {
          ...
          //  this.DbContext.InitializeDatabase();
        }
        ...
        public override byte[] GetData(string url)
        {
            ...
            if (ReportsFactory.Reports.ContainsKey(reportName))
            {
                ...
                string parameterQuery = parts.Length > 1 ? parts[1]:string .Empty;
                var parameters=HttpUtility.ParseQueryString(parameterQuery);
                ...
                {
                    if(paramName=="Id")
                    report.Parameters[paramName].Value =Guid.Parse(parameters.Get(paramName));
                    else
                    {
                        report.Parameters[paramName].Value = parameters.Get(paramName);
